...
test_that("cache #17", {
    ...
    expect_silent(dockerize(rang_ok, output_dir = temp_dir, cache = TRUE, verbose = FALSE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE))
    x <- readLines(file.path(temp_dir, "Dockerfile"))
...
test_that("cache for R < 3.1 and R >= 2.1", {
    ...
    temp_dir <- .generate_temp_dir()
    dockerize(rang_rio, output_dir = temp_dir, cache = TRUE, verbose = FALSE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE)
    x <- readLines(file.path(temp_dir, "Dockerfile"))
    expect_true(any(grepl("^COPY cache", x)))
    expect_true(dir.exists(file.path(temp_dir, "cache/rpkgs")))
...
test_that("Non-cran must enforce caching ref #22", {
    ...
    expect_error(dockerize(graph, output_dir = temp_dir, verbose = FALSE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE)) ## cache = FALSE
    expect_error(dockerize(graph, output_dir = temp_dir, cache = TRUE, verbose = FALSE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE), NA)
    temp_dir <- .generate_temp_dir()
    graph <- readRDS("../testdata/anciente1071.RDS")
    expect_equal(graph$ranglets[[1]]$pkgref, "cran::e1071")
    expect_error(dockerize(graph, output_dir = temp_dir, verbose = FALSE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE), NA)
    expect_error(dockerize(graph, output_dir = temp_dir, cache = TRUE, verbose = FALSE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE), NA)
})
...
test_that("cache bioc pkgs", {
    ...
    expect_silent(dockerize(rang_bioc, output_dir = temp_dir, cache = TRUE, verbose = FALSE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE))
    x <- readLines(file.path(temp_dir, "Dockerfile"))
    expect_true(any(grepl("^COPY cache", x)))
    expect_true(dir.exists(file.path(temp_dir, "cache/rpkgs")))
...
test_that("issue 89", {
    ...
    temp_dir <- .generate_temp_dir()
    expect_error(dockerize(x, output_dir = temp_dir, cache = TRUE, verbose = FALSE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE), NA)
...
test_that("skip_r17", {
    ...
    temp_dir <- .generate_temp_dir()
    expect_error(dockerize(graph, output_dir = temp_dir, cache = TRUE, verbose = FALSE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE), NA) ## skip_r17 = TRUE
    x <- readLines(file.path(temp_dir, "Dockerfile"))
    expect_true(any(grepl("^RUN bash \\$COMPILE_PATH 1\\.8\\.0", x)))
    expect_error(dockerize(graph, output_dir = temp_dir, cache = TRUE, verbose = FALSE, skip_r17 = FALSE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE), NA)
    x <- readLines(file.path(temp_dir, "Dockerfile"))
...
test_that("dockerize with inst/rang", {
    ...
    dockerize(rang_ok, output_dir = temp_dir, verbose = FALSE, cache = TRUE, cran_mirror = "https://cran.r-project.org/", check_cran_mirror = FALSE)
