...
test_that("normal", {
    ...
    skip_if_offline()
    skip_on_cran()
    expect_silent(x2 <- resolve("LDAvis", snapshot_date = "2023-01-01"))
    expect_equal(x2$unresolved_pkgs, character(0))
...
test_that("unresolved", {
    ...
    warns <- capture_warnings(res <- resolve("LDAvis", snapshot_date = "2000-10-01"))
    ...
    warns2 <- capture_warnings(x3 <- resolve(c("sna", "LDAvis"), snapshot_date = "2001-10-01"))
    expect_true(length(warns2) == 1)
    expect_true(any(grepl("^Some package", warns2)))
    expect_false(any(grepl("^No packages to query", warns2)))
    expect_equal(x3$unresolved_pkgs, "LDAvis")
    expect_false(length(x3$grans) == 0)
    expect_equal(x3$deps_sysreqs, character(0))
})
